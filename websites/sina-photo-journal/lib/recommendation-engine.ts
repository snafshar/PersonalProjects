import type { GearProfile, LightLevel, Recommendation, Scenario, ShootingContext } from '@/lib/photography';
...
export function generateRecommendation(scenario: Scenario, context: ShootingContext, gear: GearProfile): Recommendation {
  ...
  const focal = `${scenario.focalRange[0]}–${scenario.focalRange[1]} mm typical for this scenario; current lens ${gear.focalLength} mm`;
  ...
  const groups = [
    ...
    },
    {
      title: 'Lighting & flash', icon: 'ϟ', parameters: [
        { label: 'Flash approach', value: useFlash ? scenario.flash : !context.flashAllowed ? 'No flash permitted' : !gear.flashAvailable ? 'Available light; flash equipment not selected' : scenario.flash, reason: useFlash ? 'Adds controlled light without overwhelming the ambient scene.' : 'Respects the scenario and current constraint.' },
